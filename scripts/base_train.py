...
parser.add_argument("--idn-stride", type=int, default=0, help="IDN reg loss stride: 0=last layer only (original), S>0=add loss every S layers via full forward substitution")
parser.add_argument("--idn-detach-target", action="store_true", help="Detach sequential target in IDN/mHC Newton reg loss (stop gradient through target)")
...
parser.add_argument("--use-mhc", action="store_true", help="enable mHC residual connections (multi-stream with doubly stochastic mixing)")
...
# mHC implies no_x0_resid (mHC handles its own residual routing)
if args.use_mhc and not args.no_x0_resid:
    args.no_x0_resid = True
...
model.idn_stride = args.idn_stride
model.idn_detach_target = args.idn_detach_target
...
while True:
    ...
    for micro_step in range(grad_accum_steps):
        ...
        loss = model(x, y, jacobi_reg=args.jacobi_reg, identity_newton_reg=args.identity_newton_reg, diag_newton_reg=args.diag_newton_reg, mhc_newton_reg=args.mhc_newton_reg)
