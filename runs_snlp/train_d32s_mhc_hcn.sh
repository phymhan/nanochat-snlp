#!/bin/bash
# Train: Nanochat-0.5B mHC HCN reg (d32s, mHC x0+VE, lambda=0.5, stride=0, detach)
# Model tag: d32s_mhc4_x0ve_newton05, 4800 steps
set -e
...
PYTHONUNBUFFERED=1 torchrun --standalone --nproc_per_node=${NGPU:-4} \
    ...
    --mhc-newton-reg=0.5 --idn-detach-target --jacobi-reg-warmup=0.2 \
    --n-par-configs=8,16,24 \
    --model-tag=d32s_mhc4_x0ve_newton05 \
    --run=dummy \
    2>&1 | tee "$NANOCHAT_BASE_DIR/d32s_mhc4_x0ve_newton05_train.log"
